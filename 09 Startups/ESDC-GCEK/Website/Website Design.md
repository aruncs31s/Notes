...
This layer contains the core business logic and domain models. It is responsible for defining the business rules and entities that are central to the application. In our case, this includes:
...
- **Entities** (Aggregate Roots)
    - `User.ts` - User entity with 20+ business methods
    - `Challenge.ts` - Challenge entity with submission logic
    - `Project.ts` - Project entity with status management
    - `Event.ts` - Event entity with registration logic

- **Domain Services**
- `UserRegistrationService.ts` - User registration business logic
- `ChallengeEvaluationService.ts` - Challenge submission/completion
- `LeaderboardService.ts` - Ranking and points calculation

✅ **Domain Events**
- `DomainEvents.ts` - 10 domain events (UserCreated, ChallengeCompleted, etc.)
...
- `ChallengeRepository.ts` - Challenge data access
...
- `ApplicationService.ts` - Simplified API for UI components

-  **Dependency Injection**
    - `Container.ts` - Service container for dependency management
...
```mermaid
flowchart TD
    A["Presentation Layer (UI)<br/>components/, pages/"] --> B["Application Layer<br/>ApplicationService (Facade)<br/>Use Cases, Container"]
    B --> C["Domain Layer<br/>Entities<br/>Value Objects<br/>Services<br/>Events"]
    B --> D["Infrastructure Layer<br/>Repositories<br/>API Client"]
    D --> C
```
